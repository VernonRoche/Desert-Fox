...
export function loadEntitiesAndMap(game: Game): Map<string, Entity> {
    ...
    let allEntities = new Map<string, Entity>();
  ...
  function addUnitsToPlayer(player: Player, units: UnitJson[], allEntities : Map<string, Entity>): void {
  ...
  function addDumpsToPlayer(player: Player, dumps: DumpJson[], allEntities : Map<string, Entity>){
    ...
    function addSupplyUnitsToPlayer(player: Player, supplyUnits: SupplyUnitJson[], allEntities : Map<string, Entity>){
        supplyUnits.forEach((supplyUnit: SupplyUnitJson) => {
            ...
            if (isNaN(x) || isNaN(y)) {
              ...
              throw new Error("Error loading supply unit: " + supplyUnitId + " " + supplyUnit.currentPosition);
            }
            const supplyUnitToAdd = new SupplyUnit(supplyUnitId, new HexID(y, x), supplyUnit.movementPoints)
